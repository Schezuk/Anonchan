...
class SqlStmt {
	...
	public static $QUERY_STATEMENT = array(
				...
				'DADY_UPDATE'  => 'UPDATE `{$TABLENAME}` SET
									...
									WHERE `id` = :parent;',
				'ROOT_UPDATE'  => 'UPDATE `{$TABLENAME}` SET `content` = LEFT(CONCAT(CHAR(:createdAt),:uid,`content`), {$LEN_REPLYRECORD}) WHERE `id` = 0;',
				# READ
				'NODE_SELECT'  => 'SELECT * FROM `{$TABLENAME}` WHERE `id` = :id;',
				'SONS_SELECT'  => 'SELECT * FROM `{$TABLENAME}` WHERE `id` IN ({$NODES});',
				'PAGE_SELECT'  => 'SELECT * FROM `{$TABLENAME}` WHERE `parent` = :id ORDER BY `updatedAt` DESC LIMIT :limit , :offset;',
				# EDIT
				...
				'POST_LIKE'    => 'UPDATE `{$TABLENAME}` SET
								    ...
								    `like` = `like` + 1,
									`liker` = LEFT(CONCAT(:uid, ",", `liker`), {$LEN_LIKER})
				...
				'ADMIN_EDIT'   => 'UPDATE `{$TABLENAME}` SET `content`= CONCAT("name: ", `name`, "<br />content: ", `content`, "<br />edit time:", NOW(), "<br /><hr />", :content) WHERE `id` = :id;',
